...
class MinuteView: UIView, CAAnimationDelegate {
    ...
    let smallInnerRadius: CGFloat = 30
    let smallOuterRadius: CGFloat = 35
...
//            if j % 10 == 0 {
                ...
                let inner = CGPoint(x: smallInnerRadius * cos(angle)+200, y: smallInnerRadius * sin(angle)+130)
                let outer = CGPoint(x: smallOuterRadius * cos(angle)+200, y: smallOuterRadius * sin(angle)+130)
                path.move(to: inner)
                path.addLine(to: outer)
                UIColor.white.set()
                path.stroke()
